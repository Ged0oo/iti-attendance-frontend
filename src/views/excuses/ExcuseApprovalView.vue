  ...
  <main class="flex-1 overflow-y-auto p-margin-mobile md:p-margin-desktop bg-canvas min-h-screen">
    <div class="max-w-6xl mx-auto space-y-8">
      ...
      <header class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div class="flex items-center gap-2 bg-surface px-4 py-2 rounded-lg border border-outline-variant shadow-soft">
      ...
      <div v-else-if="pendingExcuses.length === 0" class="bg-surface rounded-2xl p-12 text-center shadow-soft border border-outline-variant/30">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div 
          ...
          :key="excuse.id"
          class="bg-surface rounded-2xl shadow-elevated border border-outline-variant/30 overflow-hidden flex flex-col"
        >
...
// Base URL for Laravel storage link
